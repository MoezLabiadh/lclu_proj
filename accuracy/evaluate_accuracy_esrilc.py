...
def sample_raster_values(raster_fp, coords):
    ...
    Filters out predictions not between 1 and 9 and excludes class 2.
    ...
    valid_indices = [i for i, p in enumerate(predictions) if 1 <= p <= 9 and p != 2]
...
if __name__ == '__main__':
    ...
    #raster_fp = os.path.join(wks, 'classification', 'mosaic', 'mosaic_south.tif')
    raster_fp = os.path.join(wks, 'validation', 'reclassified', 'esri_reclass.tif')

    # Mapping from numeric IDs to descriptive class labels.
    class_label_mapping = {
        1: 'Tree cover',
        # Class 2 (Shrubland) has been removed.
        3: 'Grassland',
        4: 'Cropland',
        5: 'Wetland',
        6: 'Permanent Water',
    ...
    # Update the labels to exclude class 2.
    numeric_labels = [1, 3, 4, 5, 6, 7, 8, 9]
    ...
    # Filter out evaluation points corresponding to class 2 (Shrubland)
    filtered_indices = [i for i, c in enumerate(ground_truth_numeric) if c != 2]
    ground_truth_numeric = [ground_truth_numeric[i] for i in filtered_indices]
    coords = [coords[i] for i in filtered_indices]
    ...
    heatmap_path = os.path.join(wks, 'validation', 'esri_heatmap_confusion_matix.png')
    plot_confusion_heatmap(cm_df, heatmap_path)

    # Plot and save the classification report heatmap.
    classification_heatmap_path = os.path.join(wks, 'validation', 'esri_heatmap_classification_report.png')
    ...
    excel_file_path = os.path.join(os.path.dirname(heatmap_path), 'esri_accuracy_assesement_summary.xlsx')
